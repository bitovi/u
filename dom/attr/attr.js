...
var setImmediate = u.global.setImmediate || function (cb) {
		return setTimeout(cb, 0);
	},
	attr = {
		// This property lets us know if the browser supports mutation observers.
		// If they are supported then that will be setup in u/util/jquery and those native events will be used to inform observers of attribute changes.
		// Otherwise this module handles triggering an `attributes` event on the element.
		MutationObserver: u.global.MutationObserver || u.global.WebKitMutationObserver || u.global.MozMutationObserver,

		/**
		 * @property {Object.<String,(String|Boolean|function)>} u.view.attr.map
		 * @parent u.view.elements
		 * @hide
		 *
		 *
		 * A mapping of
		...
		set: function (el, attrName, val) {
			...
			if (typeof prop === "function") {
				...
				if (attrName === "checked" && el.type === "radio") {
					if (u.inArray(tagName, attr.defaultValue) >= 0) {
				...
				if (prop === "value" && u.inArray(tagName, attr.defaultValue) >= 0) {
					el.defaultValue = val;
				}
			} else {
		...
		trigger: function (el, attrName, oldValue) {
			if (u.data(u.$(el), "canHasAttributesBindings")) {
				attrName = attrName.toLowerCase();
		...
		has: (function () {
			var el = u.global.document && document.createElement('div');
