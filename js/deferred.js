...
u.Deferred = Deferred;
u.when = Deferred.when = function () {
	var args = u.makeArray(arguments);
	if (args.length < 2) {
		var obj = args[0];
		if (obj && (u.isFunction(obj.isResolved) && u.isFunction(obj.isRejected))) {
			return obj;
		} else {
			return Deferred()
				.resolve(obj);
...
var resolveFunc = function (type, _status) {
	return function (context) {
		...
		return function () {
			...
			// so we have to convert arguments to an `Array` that allows `u.each` to loop over them.
			u.each(Array.prototype.slice.call(arguments), function (v, i, args) {
				if (!v) {
					return;
				}
...
var wire = function(parentDeferred, result, setter, value){
	if( isDeferred(result) ) {
		result.done(u.proxy(parentDeferred.resolve, parentDeferred))
			.fail( u.proxy(parentDeferred.reject, parentDeferred) );
	} else {
